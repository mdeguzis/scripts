...
# Provides a general agnostic setup for Termux, including the current best
# Claude Code workaround on native Termux plus a proot fallback.
...
setup_termux_tmp_env() {
	...
	for env_file in "${HOME}/.profile" "${HOME}/.zshenv"; do
		touch "${env_file}"
		append_if_missing "${env_file}" '# Added by termux/setup-termux.sh for Claude Code on Termux'
...
# Keep Claude Code usable on native Termux by:
# 1. exporting TMPDIR/CLAUDE_CODE_TMPDIR to writable Termux locations
# 2. symlinking system ripgrep into Claude's missing arm64-android vendor slot
# 3. patching hardcoded /tmp references in cli.js when needed
